...
dico_temp = {}
for i in range(1, len(gare_dico) - 1):
    for cle, valeur in gare_dico[i].items():
        for j in range(len(valeur)):
            if valeur[j - 1] not in dico_temp:
                dico_temp[valeur[j - 1]] = [cle]
            else:
                dico_temp[valeur[j - 1]].append(cle)
ligne_dico = [{ligne: gares} for ligne, gares in dico_temp.items()]
# for i in range(len(ligne_dico)-1):
#     for cle,valeur in ligne_dico[i].items():

#         if cle=="METRO 8METRO 13" or cle=="METRO 3METRO 12":
#             print("cle : ", cle," gare : ", valeur)
"""
Remarques :
    beug :
        on a :  {METRO 8METRO 13   :  ['Invalides']} {METRO 3METRO 12 :
        ['Saint-Lazare']}
        """
